...
LAW_ONLY_MODE = os.getenv("GTI_LAW_ONLY", "N").strip().upper() in {
    "Y", "YES", "TRUE", "1"
}
REQUIRE_FRESH_NEWS = os.getenv("GTI_REQUIRE_FRESH_NEWS", "N").strip().upper() in {
    "Y", "YES", "TRUE", "1"
}
...
def _balanced_mail_widths(rows: pd.DataFrame) -> list[float]:
    """Allocate text-column width from average displayed length.

    Short classification columns keep fixed compact widths. The remaining
    width is distributed across headline/summary/impact/action so their
    expected wrapped line counts stay reasonably close in Outlook HTML.
    """
    text_cols = ["Headline", "Major Changes", "AI Analysis", "Action Plan"]
    minimums = [12.0, 18.0, 16.0, 15.0]
    budget = 67.0

    averages = []
    for col in text_cols:
        values = rows.get(col, pd.Series(dtype=str)).fillna("").astype(str)
        if values.empty:
            averages.append(1.0)
            continue
        # CJK characters occupy about twice the visual width of Latin text.
        display_lengths = values.map(lambda s: sum(2 if ord(ch) > 127 else 1 for ch in s))
        averages.append(max(1.0, float(display_lengths.mean())))

    remaining = budget - sum(minimums)
    excess = [max(0.0, avg - minimums[i] * 12.0) for i, avg in enumerate(averages)]
    total_excess = sum(excess)
    if total_excess <= 0:
        additions = [remaining / 4.0] * 4
    else:
        additions = [remaining * x / total_excess for x in excess]
    text_widths = [minimums[i] + additions[i] for i in range(4)]

    # No, Issue, Headline, Summary, Samsung impact analysis, Action,
    # Country, Risk, Impact grade, Publish Date = 100%.
    return [3.0, 6.0, text_widths[0], text_widths[1], text_widths[2],
            text_widths[3], 7.0, 4.0, 5.0, 8.0]
...
def table_html(title: str, rows: pd.DataFrame, color: str) -> str:
    ...
    for _, row in rows.iterrows():
        ...
        """)
    widths = _balanced_mail_widths(rows)
    colgroup = "".join(f'<col style="width:{w:.1f}%;">' for w in widths)
    return f"""
    <h3 style="margin-top:24px;color:{color};">{html.escape(title)} ({len(rows)}건)</h3>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;width:100%;font-size:12px;line-height:1.45;table-layout:fixed;word-break:break-word;overflow-wrap:anywhere;">
...
def save_excel(rows: pd.DataFrame, top3: pd.DataFrame, paths: dict[str, Path]) -> None:
    ...
    if paths["cumulative"].exists():
        try:
            ...
            old_cumulative = _v322_recalibrate_news(old_cumulative)
        except Exception:
            old_cumulative = pd.DataFrame()
    else:
        old_cumulative = pd.DataFrame()
    cumulative = pd.concat([old_cumulative, rows], ignore_index=True, sort=False)
    for col in OUTPUT_COLUMNS:
        if col not in cumulative.columns:
            cumulative[col] = ""
    if not cumulative.empty:
        cumulative["_cum_key"] = cumulative.apply(
            lambda r: "|".join([
                clean(r.get("Content Type")).lower(),
                clean(r.get("URL")).lower(),
                clean(r.get("Headline")).lower(),
                clean(r.get("Date"))[:10],
            ]), axis=1,
        )
        cumulative = cumulative.drop_duplicates("_cum_key", keep="last").drop(columns="_cum_key")
    cumulative[OUTPUT_COLUMNS].to_excel(paths["cumulative"], index=False)
...
def save_excel(rows: pd.DataFrame, top3: pd.DataFrame, paths: dict[str, Path]) -> None:
    """Save daily report and append event-level history without overwriting cumulative."""
    rows = ensure_output_columns_v19(rows)
    ...
    if paths["cumulative"].exists():
        try:
            ...
            raise RuntimeError(
                ...
                f"{paths['cumulative']} / {type(exc).__name__}: {exc}"
            )
    else:
        old_cumulative = pd.DataFrame(columns=OUTPUT_COLUMNS)

    cumulative_parts = [frame for frame in (old_cumulative, rows) if not frame.empty]
    combined = (
        pd.concat(cumulative_parts, ignore_index=True, sort=False)
        if cumulative_parts else pd.DataFrame(columns=OUTPUT_COLUMNS)
    )
    for col in OUTPUT_COLUMNS:
        if col not in combined.columns:
            combined[col] = ""

    def cumulative_event_key(r: pd.Series) -> str:
        content_type = clean(r.get("Content Type")).lower()
        cluster = clean(r.get("Cluster")).lower()
        url = clean(r.get("URL")).lower()
        headline = re.sub(r"\s+", " ", clean(r.get("Headline")).lower()).strip()
        if content_type == "news":
            semantic = _v321_news_semantic_key(r)
            identity = "event:" + (semantic or cluster or headline)
        elif "전략물자수출입고시" in headline:
            identity = "event:kr_strategic_goods_notice_2026_101"
        elif "아크릴산" in headline and "부틸" in headline and any(
            x in headline for x in ["덤핑", "관세", "anti-dumping", "antidumping"]
        ):
            identity = "event:kr_cn_butyl_acrylate_ad_regulation"
        else:
            identity = "url:" + url if url else "headline:" + headline
        return content_type + "|" + identity

    before = len(combined)
    combined["_cum_key"] = combined.apply(cumulative_event_key, axis=1)
    combined = combined.drop_duplicates("_cum_key", keep="last").drop(columns="_cum_key")
    combined[OUTPUT_COLUMNS].to_excel(paths["cumulative"], index=False)
    print(
        f"[STEP5 CUMULATIVE] old={len(old_cumulative)} + daily={len(rows)} "
        f"-> {len(combined)} / duplicate_removed={before-len(combined)}"
    )
...
for _reg_map_col in [
    ...
    if _reg_map_col not in OUTPUT_COLUMNS:
        OUTPUT_COLUMNS.append(_reg_map_col)
...
def normalize_input(df: pd.DataFrame, content_type: str, source_file: Path) -> pd.DataFrame:
    ...
    audit_map = {
        ...
        # STEP4 decision/audit fields required by the active v319 Top3 and
        # semantic handoff. Dropping these made Top3=0 despite eligible rows.
        "AIRelevant": ["AIRelevant", "ai_relevant"],
        "AIRelevanceScore": ["AIRelevanceScore", "ai_relevance_score"],
        "Top3 Eligible": ["Top3 Eligible", "Top3Eligible", "top3_eligible"],
        "Body Verified": ["Body Verified", "BodyVerified", "body_verified"],
        "Direct Evidence": ["Direct Evidence", "DirectEvidence", "direct_evidence"],
        "Missing Facts": ["Missing Facts", "MissingFacts", "missing_facts"],
        "Policy Event": ["Policy Event", "PolicyEvent", "policy_event"],
        "Official Evidence": ["Official Evidence", "OfficialEvidence", "official_evidence"],
        "AIReason": ["AIReason", "AI Reason", "ai_reason"],
        "Cluster": ["Cluster", "SemanticEventKey", "semantic_event_key"],
        "RegulationMappingType": ["RegulationMappingType", "regulation_mapping_type"],
        "MappingStatus": ["MappingStatus", "mapping_status"],
        "RequiredMappingKeys": ["RequiredMappingKeys", "required_mapping_keys"],
        "EntityDirectFlag": ["EntityDirectFlag", "entity_direct_flag"],
        "MappedEntity": ["MappedEntity", "mapped_entity"],
        "MappedProduct": ["MappedProduct", "mapped_product"],
        "MappedHS": ["MappedHS", "mapped_hs"],
        "TradeRoute": ["TradeRoute", "trade_route"],
        "MappingEvidence": ["MappingEvidence", "mapping_evidence"],
        "SamsungRelevanceScore": ["SamsungRelevanceScore", "samsung_relevance_score"],
        "CustomsTradePolicyScore": ["CustomsTradePolicyScore", "PolicyScore", "customs_trade_policy_score"],
        "DirectImpactScore": ["DirectImpactScore", "direct_impact_score"],
...
# ======================================================================
# GTI STEP5 v319: clean STEP4 handoff
# ----------------------------------------------------------------------
# STEP4-1 and STEP4-2 are the selection owners.  STEP5 must not silently
# re-select their daily outputs with legacy title/body/URL guards.  It only:
#   * preserves the source branch type (Regulation vs News),
#   * applies a final 24-hour news freshness check,
#   * removes exact duplicates,
#   * formats and ranks the already-selected rows for the report.
...
for _v330_col in [
    ...
    "Policy Stage", "Quality Contract", "Verification Group",
]:
...
def _v319_parse_news_date(row: pd.Series):
    """Return (timestamp, is_date_only) using the best published-date field."""
    for col in ["Publish Date", "Date", "Published", "published"]:
        raw = clean(row.get(col))
        if not raw or raw.lower() in {"nan", "none", "nat", "null", "?"}:
            continue
        date_only = bool(re.fullmatch(r"\d{4}[-./]\d{1,2}[-./]\d{1,2}", raw[:10])) and len(raw) <= 10
        dt = pd.to_datetime(raw, errors="coerce")
        if not pd.isna(dt) and dt.year > 1970:
            return pd.Timestamp(dt), date_only
    return None, False
...
def _v319_news_fresh(row: pd.Series, now: pd.Timestamp) -> tuple[bool, str]:
    dt, date_only = _v319_parse_news_date(row)
    if dt is None:
        return False, "MISSING_PUBLISH_DATE"
    if dt > now + pd.Timedelta(minutes=10):
        return False, "FUTURE_PUBLISH_DATE"
    cutoff = now - pd.Timedelta(hours=24)
    if date_only:
        # STEP4-2 already performed the exact 24-hour check.  When its output
        # contains only YYYY-MM-DD, retain yesterday/today rather than inventing
        # midnight and incorrectly dropping a valid article.
        return (dt.date() >= cutoff.date(), "DATE_ONLY_ACCEPTED" if dt.date() >= cutoff.date() else "STALE_DATE")
    return (dt >= cutoff, "FRESH" if dt >= cutoff else "STALE_OVER_24H")


def _v321_event_only_news(row: pd.Series) -> bool:
    title = clean(row.get("Headline")).lower()
    body = " ".join([title, clean(row.get("Summary")).lower(), clean(row.get("AI Analysis")).lower()])
    event_terms = [
    ...
    measure_terms = [
        ...
        "effective", "entered into force", "imposed", "amended",
        "investigation initiated", "preliminary determination", "final determination",
    ]
    return any(x in title for x in event_terms) and not any(x in body for x in measure_terms)


def _v321_news_semantic_key(row: pd.Series) -> str:
    title = clean(row.get("Headline")).lower()
    text = " ".join([
        clean(row.get("Headline")), clean(row.get("Summary")),
        clean(row.get("Country")),
    ]).lower()
    # Stable policy-event identities for recurring multi-source coverage.
    # Keep one representative article instead of filling the report with
    # multiple rewrites of the same government measure or political signal.
    if all(x in text for x in ["아크릴산", "부틸"]) and any(
        x in text for x in ["덤핑", "anti-dumping", "antidumping", "duty", "관세"]
    ):
        return "KR_CN_BUTYL_ACRYLATE_AD_EVENT"
    if "쿠팡" in text and any(x in text for x in ["관세", "301조", "section 301", "통상"]):
        return "KR_US_COUPANG_TARIFF_TENSION"
    if "추석" in text and any(x in text for x in ["원산지", "국산 둔갑", "특별단속"]):
        return "KR_CHUSEOK_ORIGIN_MARKING_ENFORCEMENT"
    if "전략물자수출입고시" in text:
        return "KR_STRATEGIC_GOODS_NOTICE_2026_101"
    semicon = [
        ["미국", "트럼프", "백악관", "washington", "u.s."],
        ["삼성전자", "삼전", "samsung", "sk하이닉스", "하이닉스", "반도체", "semiconductor", "chip"],
        ["관세", "tariff", "투자 압박", "투자 이행", "미국 투자", "유치 압박", "양자택일", "편 서지 마", "pressure"],
    ]
    if (all(any(term in text for term in group) for group in semicon[:1] + semicon[2:])
            and any(term in title for term in semicon[1])):
        return "US_SEMICON_TARIFF_INVESTMENT_PRESSURE"
    drone = [
        ["미국", "트럼프", "u.s."], ["드론", "drone", "무인기", "uas"],
        ["100%", "100％", "최대 100", "section 232", "232조"], ["관세", "tariff"],
    ]
    if (all(any(term in text for term in group) for group in drone[:1] + drone[2:])
            and any(term in title for term in drone[1])):
        return "US_DRONE_232_TARIFF"
    if (
        any(x in text for x in ["미국", "美", "미,", "미-", "미·", "미 캐나다", "미 관세", "usa", "us ", "u.s.", "united states"])
        and any(x in text for x in ["캐나다", "canada"])
        and any(x in text for x in ["관세", "tariff"])
    ):
        return "US_CANADA_50PCT_RETALIATORY_TARIFFS"
    if (
        any(x in text for x in ["호우", "침수", "수해", "flood"])
        and any(x in text for x in ["관세", "세관", "customs", "납부기한", "관세조사", "원산지검증", "신속통관", "통관 지원", "지원책"])
    ):
        return "KR_FLOOD_CUSTOMS_RELIEF"
    return clean(row.get("Cluster")) or clean(row.get("Headline")).lower()


def read_step4_results() -> pd.DataFrame:
    """Read only the two official daily STEP4 outputs, without legacy guards."""
    frames = []
    if REGULATION_INPUT_FILE.exists():
        raw_reg = pd.read_excel(REGULATION_INPUT_FILE)
        ...
        print(f"[STEP5 WARN] regulation input missing: {REGULATION_INPUT_FILE}")
    ...
    if use_news:
        ...
        if file_age_hours > max_input_age_hours:
            message = (
                f"STEP4-2 news output is stale ({file_age_hours:.1f}h > {max_input_age_hours}h): "
                f"{NEWS_INPUT_FILE}"
            )
        ...
        if use_news and step32_input.exists() and NEWS_INPUT_FILE.stat().st_mtime < step32_input.stat().st_mtime:
            message = (
                f"STEP4-2 news output is older than STEP3-2 input: {NEWS_INPUT_FILE.name} < "
                f"{step32_input.name}"
            )
            if REQUIRE_FRESH_NEWS:
                raise RuntimeError(message + ". Run 4-2 successfully before STEP5.")
            print(f"[STEP5 OLD NEWS SKIP] {message}; regulation report continues")
            use_news = False

    if use_news:
        raw_news = pd.read_excel(NEWS_INPUT_FILE)
        required = {"AIRelevant", "AIRelevanceScore", "Body Verified", "Policy Event"}
        missing = sorted(required - set(raw_news.columns))
        if missing:
            raise RuntimeError(f"STEP4-2 quality fields missing: {missing}")
        score_ok = pd.to_numeric(raw_news["AIRelevanceScore"], errors="coerce").fillna(0).ge(3)
        verified = raw_news["Body Verified"].astype(str).str.upper().eq("Y")
        strict_selected = (
        ...
        news["Content Type"] = "News"
        news["Mail Group"] = GROUP_NEWS
        event_mask = news.apply(_v321_event_only_news, axis=1)
        if event_mask.any():
            print(f"[STEP5 EVENT-ONLY GUARD] removed={int(event_mask.sum())}")
            news = news.loc[~event_mask].copy()
        if NEWS_MAX_ROWS > 0:
            news = news.head(NEWS_MAX_ROWS).copy()

        now = pd.Timestamp.now()
        freshness = news.apply(lambda r: _v319_news_fresh(r, now), axis=1)
        news["FreshnessStatus"] = freshness.apply(lambda x: x[1])
        keep = freshness.apply(lambda x: x[0])
        dropped = news.loc[~keep].copy()
        news = news.loc[keep].copy()
        print(
            f"[STEP5 24H GUARD] news={len(raw_news)} / fresh={len(news)} / "
            f"excluded={len(dropped)} / hours=24"
        )
        if not dropped.empty:
            for _, row in dropped.head(20).iterrows():
                print(
                    f"  - {clean(row.get('Headline'))[:120]} / "
                    f"date={clean(row.get('Date'))} / reason={clean(row.get('FreshnessStatus'))}"
                )
        frames.append(news)
    else:
        if LAW_ONLY_MODE:
            print("[STEP5 LAW-ONLY] news is intentionally excluded; regulation report continues")
        elif NEWS_INPUT_FILE.exists():
            print("[STEP5 INFO] news excluded from this run; regulation report continues")
        else:
            print(f"[STEP5 WARN] news input missing: {NEWS_INPUT_FILE}; regulation report continues")

    if not frames:
        raise FileNotFoundError(f"STEP4 outputs not found: {REGULATION_INPUT_FILE}, {NEWS_INPUT_FILE}")

    nonempty_frames = [frame for frame in frames if not frame.empty]
    rows = pd.concat(nonempty_frames or frames, ignore_index=True, sort=False)
    rows["_exact_key"] = rows.apply(
        lambda r: (
    ...
    before = len(rows)
    rows = rows.drop_duplicates(subset=["_exact_key"], keep="first").drop(columns=["_exact_key"], errors="ignore")
    if len(rows) != before:
        print(f"[STEP5 EXACT DEDUP] before={before} / after={len(rows)} / removed={before-len(rows)}")
    ...
    if "Cluster" in rows.columns:
        ...
        if len(rows) != before_semantic:
            print(f"[STEP5 SEMANTIC DEDUP] removed={before_semantic-len(rows)}")
...
def _v322_hard_scope_excluded(row: pd.Series) -> bool:
    if clean(row.get("Content Type")) != "News":
        return False
    title = clean(row.get("Headline")).lower()
    hard = [
    ...
    concrete = [
        "section 232", "section 301", "232조", "301조", "반덤핑", "상계관세", "세이프가드",
        "예비판정", "최종판정", "조사 개시", "관세 부과", "관세 인상", "수출통제",
        "anti-dumping", "countervailing", "safeguard", "tariff imposed", "export control",
    ]
    return any(x in title for x in hard) or (
        any(x in title for x in macro) and not any(x in title for x in concrete)
    )


def _v322_recalibrate_news(rows: pd.DataFrame) -> pd.DataFrame:
    if rows.empty:
        return rows
    rows = rows.copy()
    if "MappedHS" in rows.columns:
        rows["MappedHS"] = rows["MappedHS"].astype("object")
    # pandas 2.2+/3.x may infer Excel columns containing blank/text values as
    # StringDtype. Assigning numeric downgrade scores (35/65) then raises a
    # TypeError. Normalize every score column before any row-level mutation.
    numeric_score_columns = [
        "DirectImpactScore", "IndirectImpactScore", "WeightedScore",
        "CustomsTradeLawScore", "CustomsTradePolicyScore",
        "SamsungRelevanceScore", "Importance Score",
    ]
    normalized_score_columns = []
    for col in numeric_score_columns:
        if col in rows.columns:
            rows[col] = pd.to_numeric(rows[col], errors="coerce").fillna(0).astype("float64")
            normalized_score_columns.append(col)
    if normalized_score_columns:
        print(f"[STEP5 SCORE DTYPE NORMALIZED] columns={len(normalized_score_columns)}")
    noise = rows.apply(_v322_hard_scope_excluded, axis=1)
    if noise.any():
        print(f"[STEP5 BUSINESS-SCOPE GUARD] removed={int(noise.sum())}")
        rows = rows.loc[~noise].copy()

    valid_mapping = {"ENTITY_CONFIRMED", "ITEM_1TO1_MAPPED"}
    specific_issues = {"AD/CVD", "반덤핑/상계관세", "HS/품목분류", "FTA/원산지"}
    downgraded = 0
    indirect_downgraded = 0
    master_mapping_cleared = 0
    for idx, row in rows.iterrows():
        ...
        article_text = " ".join([
            clean(row.get("Headline")), clean(row.get("Summary")),
            clean(row.get("Original Summary")), clean(row.get("Original Body Text")),
        ]).lower()
        mapped_product = clean(row.get("MappedProduct")).lower()
        mapped_entity = clean(row.get("MappedEntity")).lower()
        samsung_in_article = bool(re.search(r"삼성|samsung", article_text, re.I))
        product_in_article = bool(
            mapped_product and len(mapped_product) >= 2 and mapped_product in article_text
        )
        mapped_hs_in_article = bool(
            mapped_hs and len(mapped_hs) >= 4
            and re.search(rf"(?<!\d){re.escape(mapped_hs.rstrip('.0'))}(?!\d)", article_text)
        )
        indirect_supported = bool(
            (status == "ENTITY_CONFIRMED" and (samsung_in_article or bool(re.search(r"삼성|samsung", direct_evidence, re.I))))
            or (
                status == "ITEM_1TO1_MAPPED"
                and (samsung_in_article or mapped_hs_in_article or bool(direct_evidence))
            )
        )
        invalid_hs = bool(re.fullmatch(r"(?:19|20)\d{2}(?:\.\d+)?", mapped_hs))
        candidate_mapping = any(
            x in " ".join([clean(row.get("MappedProduct")), mapped_hs, clean(row.get("TradeRoute"))]).lower()
            for x in ["후보", "확인 필요", "verify", "candidate"]
        )
        mapping_ok = status in valid_mapping
        if issue in specific_issues:
            mapping_ok = status == "ITEM_1TO1_MAPPED"
        if invalid_hs or candidate_mapping:
            mapping_ok = False
        # ENTITY_CONFIRMED alone is insufficient: the evidence must identify
        # Samsung in the concrete customs event, not merely in AI-generated prose.
        if status == "ENTITY_CONFIRMED":
            mapping_ok = bool(re.search(r"삼성|samsung", direct_evidence, re.I))
        if mapping_evidence == "ARTICLE_ENTITY_PRODUCT" and not direct_evidence:
            mapping_ok = False
        # A generic MASTER_MATCH must not manufacture a Samsung entity/route.
        # Retain it only when the article itself supports Samsung or the exact
        # mapped product and describes a concrete operative customs measure.
        if mapping_evidence.upper() == "MASTER_MATCH":
            concrete_measure = any(x in article_text for x in [
                "시행한다", "시행되었습니다", "발효", "부과한다", "부과되었습니다", "최종판정", "조사 개시", "수입금지",
                "effective", "entered into force", "imposed", "final determination",
                "investigation initiated", "import ban",
            ])
            third_party_only = any(x in article_text for x in [
                "현대차", "현대자동차", "토요타", "toyota", "general motors", "gm ",
            ]) and not samsung_in_article
            if third_party_only or not concrete_measure or not (
                samsung_in_article or mapped_hs_in_article or bool(direct_evidence)
            ):
                mapping_ok = False
                for col in ["MappedEntity", "MappedProduct", "MappedHS", "TradeRoute"]:
                    if col in rows.columns:
                        rows.at[idx, col] = ""
                rows.at[idx, "MappingEvidence"] = "MASTER_MATCH_REJECTED_NO_ARTICLE_LINK"
        ...
        if impact == "Direct" and not mapping_ok:
            ...
            downgraded += 1
        elif impact == "Indirect" and status in {
            "MAPPING_REQUIRED", "POLICY_MONITORING", "POLICY_REVIEW", "VERIFICATION_PENDING"
        }:
            rows.at[idx, "Samsung Impact"] = "Watch"
            rows.at[idx, "DirectImpactScore"] = 35
            rows.at[idx, "Top3 Eligible"] = "N"
            indirect_downgraded += 1
        elif impact == "Indirect" and not indirect_supported:
            rows.at[idx, "Samsung Impact"] = "Watch"
            rows.at[idx, "DirectImpactScore"] = 35
            rows.at[idx, "Top3 Eligible"] = "N"
            indirect_downgraded += 1
        top3_ok = (
            ...
            and mapping_ok
            and clean(row.get("Body Verified")).upper() == "Y"
            and bool(clean(row.get("Official Evidence")))
            and bool(clean(row.get("Direct Evidence")))
        )
        if not top3_ok:
            rows.at[idx, "Top3 Eligible"] = "N"
    if downgraded:
        print(f"[STEP5 DIRECT MAPPING GUARD] downgraded={downgraded}")
    if indirect_downgraded or master_mapping_cleared:
        print(
            f"[STEP5 INDIRECT MAPPING INTEGRITY] downgraded={indirect_downgraded} / "
            f"master_mapping_cleared={master_mapping_cleared}"
        )
    return rows


def prepare_rows(rows: pd.DataFrame) -> pd.DataFrame:
    """Format STEP4-selected rows; do not apply v22-v27 re-selection guards."""
    rows = rows.copy()
    if rows.empty:
        return rows
    rows = _v322_recalibrate_news(rows)
    if rows.empty:
        return rows
    rows["Content Type"] = rows["Content Type"].apply(lambda v: "Regulation" if clean(v) == "Regulation" else "News")
    rows["Mail Group"] = rows["Content Type"].map({"Regulation": GROUP_REGULATION, "News": GROUP_NEWS})
    rows["Issue"] = rows.apply(issue_for, axis=1)
    rows["Major Changes"] = rows.apply(major_changes, axis=1)
    rows["Summary"] = rows.apply(report_summary, axis=1)
    rows["AI Analysis"] = rows.apply(report_impact, axis=1)
...
def choose_top3(rows: pd.DataFrame) -> pd.DataFrame:
    """Return up to three genuinely material items; fewer is valid."""
    if rows.empty:
        return rows.copy()
    pool = rows.copy()
    pool["_top3_score"] = pool.apply(top3_deep_score, axis=1)
    pool["_top3_ok"] = pool.apply(
        lambda r: (
            (
                clean(r.get("Content Type")) == "News"
                ...
                and bool(clean(r.get("Official Evidence")))
                and bool(clean(r.get("Direct Evidence")))
            )
            or (
                clean(r.get("Content Type")) == "Regulation"
                and clean(r.get("Samsung Impact")) == "Direct"
                and clean(r.get("Top3 Eligible")).upper() == "Y"
                and clean(r.get("MappingStatus")) in {"ENTITY_CONFIRMED", "MAPPED", "COUNTRY_ENTITY_MAPPED", "ITEM_1TO1_MAPPED"}
            )
        ),
        axis=1,
    )
    pool = pool[pool["_top3_ok"]].sort_values(["_top3_score", "_sort_date"], ascending=[False, False])
    selected = []
    used_issues = set()
    for _, row in pool.iterrows():
        issue = clean(row.get("Issue"))
        ...
        if len(selected) == 3:
            break
    out = pd.DataFrame(selected).drop(columns=["_top3_ok"], errors="ignore").reset_index(drop=True)
    if not out.empty:
        out["No"] = range(1, len(out) + 1)
    return out


# STEP5 v324: STEP4-2 is the analysis owner.  Earlier STEP5 fallbacks replaced
# article-specific analysis/action text with generic templates, which erased
# the policy-to-Samsung decision trace.  Preserve verified STEP4 text and use
# a generic fallback only when the upstream field is genuinely empty.
def _v324_source_text(row: pd.Series, *names: str) -> str:
    for name in names:
        value = clean(row.get(name))
        if value and value.lower() not in {"nan", "none", "null", "확인 필요"}:
            return value
    return ""


def major_changes(row: pd.Series) -> str:
    source = _v324_source_text(row, "Original Summary", "Summary")
    missing = _v324_source_text(row, "Missing Facts")
    if source:
        if missing and "미확인" not in source:
            source = f"{source}\n\n[미확인/추가 확인] {missing}"
        return source
    return _v9_generic_top3_summary(row)
...
def report_summary(row: pd.Series) -> str:
    return major_changes(row)


def report_impact(row: pd.Series) -> str:
    source = _v324_source_text(row, "Original AI Analysis", "AI Analysis")
    if source:
        return source
    return _v9_generic_top3_impact(row)
...
def report_action(row: pd.Series) -> str:
    source = _v324_source_text(row, "Original Action Plan", "Action Plan")
    if source:
        return source
    return _v9_generic_top3_action(row)
...
def _issue_summary_detail(row: pd.Series) -> str:
    issue = clean(row.get("Issue")) or "확인 필요"
    country = clean(row.get("Country")) or "확인 필요"
    date = clean(row.get("Publish Date")) or clean(row.get("Date")) or "확인 필요"
    product = clean(row.get("MappedProduct")) or "미확인"
    hs = clean(row.get("MappedHS")) or "미확인"
    route = clean(row.get("TradeRoute")) or "미확인"
    evidence = clean(row.get("Official Evidence")) or "공식 근거 미확인"
    return (
        f"• 이슈/조치단계: {issue}\n"
        f"• 발표국·영향국: {country}\n"
        f"• 게시/시행일: {date}\n"
        f"• 삼성 매핑: 제품={product} / HS={hs} / 거래경로={route}\n"
        f"• 공식 근거: {evidence}\n"
        f"• 확인 사실 및 변경내역:\n{major_changes(row)}"
    )
...
# ======================================================================
# STEP5 v326 - ONE ENGINE FOR LAW + NEWS
# - Regulation: keep every STEP4-1 row and split verified/pending.
# - News: keep v325 24h, scope, semantic-event and Direct evidence guards.
# - Both branches retain their STEP4 article-specific analysis and actions.
# ======================================================================
_v326_prepare_base = prepare_rows


def _v326_verified(row: pd.Series) -> bool:
    return clean(row.get("Body Verified")).upper() == "Y"


def _v326_reg_mapping_ok(row: pd.Series) -> bool:
    status = clean(row.get("MappingStatus")).upper()
    mapped = {
        "ENTITY_CONFIRMED", "MAPPED", "COUNTRY_ENTITY_MAPPED",
        "ITEM_1TO1_MAPPED", "POLICY_GENERAL_MAPPED",
    }
    values = " ".join([
        clean(row.get("MappedEntity")), clean(row.get("MappedProduct")),
        clean(row.get("MappedHS")), clean(row.get("TradeRoute")),
    ]).lower()
    candidate = any(x in values for x in ["후보", "확인 필요", "candidate", "verify"])
    return status in mapped and not candidate


def _v332_strategic_pending(row: pd.Series) -> bool:
    """Important export-control laws need urgent visibility even without Direct proof."""
    if clean(row.get("Content Type")) != "Regulation":
        return False
    text = " ".join([
        clean(row.get("Headline")), clean(row.get("Issue")),
        clean(row.get("KeywordMatches")), clean(row.get("Impact Reason")),
    ]).lower()
    strategic = any(term in text for term in [
        "전략물자", "수출통제", "export control", "strategic goods", "entity list",
    ])
    return strategic and not _v326_verified(row)


def prepare_rows(rows: pd.DataFrame) -> pd.DataFrame:
    rows = _v326_prepare_base(rows)
    if rows.empty:
        return rows
    rows = rows.copy()
    reg_mask = rows["Content Type"].astype(str).eq("Regulation")
    rows["Verification Group"] = rows.get(
        "Verification Group", pd.Series("", index=rows.index, dtype="object")
    ).astype("object")
    rows.loc[reg_mask, "Verification Group"] = rows.loc[reg_mask].apply(
        lambda r: (
            "Verified Regulation" if _v326_verified(r)
            else "Strategic Control Urgent Verification" if _v332_strategic_pending(r)
            else "Verification Pending"
        ), axis=1
    )
    rows.loc[~reg_mask, "Verification Group"] = rows.loc[~reg_mask].apply(
        lambda r: (
            "Officially Verified Policy News"
            if _v326_verified(r) and _v330_official_primary(r.get("Official Evidence"))
            else "Article-Verified Policy News" if _v326_verified(r)
            else "News Pending"
        ), axis=1
    )
    rows["OfficialSourceFlag"] = rows.apply(
        lambda r: "Y" if _v330_official_primary(r.get("Official Evidence")) else "N", axis=1
    )

    # v334 defense in depth: AI-generated analysis text cannot sustain an
    # entity/product/HS/route mapping. Only article-native evidence is valid.
    native_mapping_downgraded = 0
    for idx, row in rows.loc[~reg_mask].iterrows():
        if clean(row.get("EntityDirectFlag")).upper() != "Y":
            continue
        native = " ".join(clean(row.get(c)) for c in [
            "Headline", "Direct Evidence", "Article Body Evidence",
        ]).lower()
        samsung_named = bool(re.search(r"삼성전자|samsung electronics|samsung semiconductor", native, re.I))
        product = clean(row.get("MappedProduct")).lower()
        product_named = bool(product and product in native)
        customs_named = any(term in native for term in [
        ...
        if not (samsung_named and product_named and customs_named):
            for col in ["MappedEntity", "MappedProduct", "MappedHS", "TradeRoute"]:
                rows.at[idx, col] = ""
            rows.at[idx, "MappingEvidence"] = "ARTICLE_NATIVE_LINK_NOT_PROVEN"
            rows.at[idx, "EntityDirectFlag"] = "N"
            rows.at[idx, "MappingStatus"] = "POLICY_REVIEW"
            rows.at[idx, "RegulationMappingType"] = "POLICY_GENERAL"
            rows.at[idx, "Top3 Eligible"] = "N"
            if clean(rows.at[idx, "Samsung Impact"]) == "Direct":
                rows.at[idx, "Samsung Impact"] = "Indirect"
            native_mapping_downgraded += 1
    if native_mapping_downgraded:
        print(f"[STEP5 ARTICLE-NATIVE MAPPING GUARD] downgraded={native_mapping_downgraded}")

    # Older STEP4 outputs may still contain obvious non-policy/third-party rows.
    news_title = rows["Headline"].fillna("").astype(str).str.lower()
    news_mask = ~reg_mask
    hard_noise = news_mask & news_title.apply(lambda title: any(term in title for term in [
        "으뜸이", "할랄시장 잡아라", "금리 압박", "수출액 늘었다고 경쟁력",
        "인사 발령", "임원 인사", "수상자", "포상",
    ]))
    unrelated_auto = news_mask & news_title.apply(
        lambda title: any(term in title for term in ["토요타", "toyota", "현대차", "hyundai"])
        and not any(term in title for term in ["삼성전자", "samsung electronics"])
    )
    remove_mask = hard_noise | unrelated_auto
    if remove_mask.any():
        print(f"[STEP5 HARD POLICY-NOISE GUARD] removed={int(remove_mask.sum())}")
        rows = rows.loc[~remove_mask].copy()
        reg_mask = rows["Content Type"].astype(str).eq("Regulation")

    # 법규는 전체를 표시하되 원문 또는 매핑이 미완료된 항목을 Direct/Top3로
    # 승격하지 않는다. 뉴스는 이미 v325의 더 엄격한 증거 게이트를 통과했다.
    for idx, row in rows.loc[reg_mask].iterrows():
        direct_ok = _v326_verified(row) and _v326_reg_mapping_ok(row)
        if clean(row.get("Samsung Impact")) == "Direct" and not direct_ok:
            rows.at[idx, "Samsung Impact"] = "Watch"
            rows.at[idx, "Top3 Eligible"] = "N"
        if not direct_ok:
            rows.at[idx, "Top3 Eligible"] = "N"

    # v334 cross-stage identity: regulation and news covering one government
    # measure compete for a single representative instead of separate sections.
    def _v334_event_key(row: pd.Series) -> str:
        text = " ".join(clean(row.get(c)) for c in [
            "Headline", "Summary", "AI Analysis", "Issue", "Cluster",
        ]).lower()
        rules = [
        ...
        if cluster and ("semantic" in cluster.lower() or cluster.upper().startswith(("KR_", "US_", "EU_", "G20_"))):
            return cluster.lower()
        return f"{clean(row.get('Content Type'))}|{cluster or clean(row.get('Headline')).lower()}"

    def _v334_rep_rank(row: pd.Series) -> float:
        group = clean(row.get("Verification Group"))
        numeric = pd.to_numeric(row.get("AIRelevanceScore"), errors="coerce")
        score = 0.0 if pd.isna(numeric) else float(numeric)
        if group == "Verified Regulation": score += 100
        elif group == "Officially Verified Policy News": score += 90
        elif group == "Article-Verified Policy News": score += 75
        elif group == "Strategic Control Urgent Verification": score += 55
        elif clean(row.get("Content Type")) == "Regulation": score += 35
        if clean(row.get("Policy Stage")) == "OPERATIVE": score += 15
        if clean(row.get("Samsung Impact")) == "Direct": score += 15
        return score

    rows["_v334_event_key"] = rows.apply(_v334_event_key, axis=1)
    rows["_v334_rep_rank"] = rows.apply(_v334_rep_rank, axis=1)
    before_cross_dedup = len(rows)
    rows = (
        rows.sort_values(["_v334_rep_rank", "_sort_date"], ascending=[False, False])
        .drop_duplicates("_v334_event_key", keep="first")
        .drop(columns=["_v334_event_key", "_v334_rep_rank"], errors="ignore")
    )
    if len(rows) != before_cross_dedup:
        print(f"[STEP5 CROSS-STAGE EVENT DEDUP] before={before_cross_dedup} / after={len(rows)} / removed={before_cross_dedup-len(rows)}")
...
def choose_policy_watch_top3(rows: pd.DataFrame) -> pd.DataFrame:
    """Actionable Indirect/Watch priorities; never relabel them Direct."""
    if rows.empty:
        return rows.copy()
    pool = rows[
        rows["Samsung Impact"].isin(["Indirect", "Watch"])
        & rows.get("Body Verified", pd.Series("N", index=rows.index)).astype(str).str.upper().eq("Y")
        & rows.get("Policy Event", pd.Series("N", index=rows.index)).astype(str).str.upper().eq("Y")
    ].copy()
    if pool.empty:
        return pool

    def _issue_family(row: pd.Series) -> str:
        text = f"{clean(row.get('Headline'))} {clean(row.get('Issue'))}".upper()
        ...
        if any(x in text for x in ["반덤핑", "덤핑관세", "덤핑 관세", "ANTI-DUMPING", "AD/CVD"]): return "AD_CVD"
        if any(x in text for x in ["FTA", "자유무역협정", "원산지 기준"]): return "ORIGIN_FTA"
        if any(x in text for x in ["품목분류", "HS CODE", "TARIFF CLASSIFICATION"]): return "HS_CLASSIFICATION"
        issue = clean(row.get("Issue")).upper().replace("/", "_")
        return {
            "수출통제": "EXPORT_CONTROL", "관세정책": "TARIFF", "FTA_원산지": "ORIGIN_FTA",
            "통관": "CUSTOMS", "품목분류": "HS_CLASSIFICATION",
        }.get(issue, issue)

    issue_bonus = {
        "EXPORT_CONTROL": 28, "AD_CVD": 24, "CBAM_CARBON": 22,
        "ORIGIN_FTA": 20, "HS_CLASSIFICATION": 20, "TARIFF": 18, "CUSTOMS": 12,
    }
...
def policy_watch_html(rows: pd.DataFrame) -> str:
    watch_top3 = choose_policy_watch_top3(rows)
    if watch_top3.empty:
        return "<div style='padding:12px;background:#F6F8FA;border-left:5px solid #7F8C8D;'>금일 검증된 Priority Policy Watch 후보가 없습니다.</div>"
    blocks = []
    for idx, row in watch_top3.iterrows():
        blocks.append(f"""
        <div style="margin:12px 0;padding:14px;border-left:5px solid #C65911;background:#FFF8E8;line-height:1.6;">
          <div style="font-size:15px;font-weight:bold;">Policy Watch {idx+1}. {html_link(row.get('Headline'), row.get('URL'))}</div>
          <div style="color:#555;margin:5px 0;">{html.escape(clean(row.get('Country')))} | {html.escape(clean(row.get('Issue')))} | {html.escape(clean(row.get('Samsung Impact')))}</div>
          <div><b>확인된 정책:</b> {html.escape(short_text(row.get('Major Changes'), '정책내용 확인 필요', 900)).replace(chr(10), '<br>')}</div>
          <div style="margin-top:6px;"><b>삼성 관세업무 검토:</b> {html.escape(short_text(row.get('AI Analysis'), '영향 검토 필요', 900)).replace(chr(10), '<br>')}</div>
          <div style="margin-top:6px;"><b>Action:</b> {html.escape(short_text(row.get('Action Plan'), '담당부서 확인 필요', 900)).replace(chr(10), '<br>')}</div>
          <div style="margin-top:5px;color:#7F6000;">Direct로 승격하지 않은 검증된 정책 우선순위입니다.</div>
        </div>""")
    return "".join(blocks)
...
def overall_html(rows: pd.DataFrame, top3: pd.DataFrame) -> str:
    reg = rows[rows["Content Type"].eq("Regulation")]
    news = rows[rows["Content Type"].eq("News")]
    verification_group = reg.get(
        "Verification Group", pd.Series("", index=reg.index, dtype="object")
    ).astype(str)
    verified_reg = reg[verification_group.eq("Verified Regulation")]
    pending_reg = reg[~reg.index.isin(verified_reg.index)]
    strategic_pending = reg[reg.apply(_v332_strategic_pending, axis=1)]
    direct = rows[rows["Samsung Impact"].eq("Direct")]
    indirect = rows[rows["Samsung Impact"].eq("Indirect")]
    watch = rows[rows["Samsung Impact"].eq("Watch")]
    issue_counts = rows["Issue"].replace("", "기타").fillna("기타").value_counts().head(6)
    country_counts = rows["Country"].replace("", "국가확인필요").fillna("국가확인필요").value_counts().head(6)
    issues = ", ".join(f"{html.escape(str(k))} {v}건" for k, v in issue_counts.items()) or "신규 핵심정책 없음"
    countries = ", ".join(f"{html.escape(str(k))} {v}건" for k, v in country_counts.items()) or "국가정보 확인 필요"
    top_lines = "".join(
        f"<li><b>{html.escape(clean(r.get('Headline')))}</b> — {html.escape(one_line(r))}</li>"
        for _, r in top3.iterrows()
    ) or "<li>Direct 근거와 실행성을 모두 충족한 Top3 후보가 없습니다.</li>"
    strategic_lines = "".join(
        f"<li><b>{html_link(r.get('Headline'), r.get('URL'))}</b> — 공식 원문·개정 전후표를 즉시 확보하여 대상 품목, ECCN/전략물자 번호, 허가요건과 시행일을 확인하십시오.</li>"
        for _, r in strategic_pending.iterrows()
    ) or "<li>금일 긴급 원문확인 대상 전략물자·수출통제 법규는 없습니다.</li>"
    return f"""
    <div style="padding:16px;background:#F4F6F8;border-left:6px solid #1F4E78;margin-bottom:18px;line-height:1.65;">
      <div style="font-size:15px;font-weight:bold;color:#1F4E78;">A. 글로벌 관세·통상 정책 Radar</div>
      <div>법규 {len(reg)}건(원문검증 {len(verified_reg)} / 검증대기 {len(pending_reg)}), 정책뉴스 {len(news)}건</div>
      <div><b>주요 정책축:</b> {issues}</div>
      <div><b>주요 국가:</b> {countries}</div>
      <div style="margin-top:12px;font-size:15px;font-weight:bold;color:#C00000;">B. 삼성전자 관세업무 영향</div>
      <div>Direct {len(direct)}건 / Indirect {len(indirect)}건 / Watch {len(watch)}건</div>
      <div>법규는 원문검증·적용범위 매핑, 뉴스는 삼성 직접증거 또는 승인된 1:1 매핑을 기준으로 판정했습니다.</div>
      <div style="margin-top:9px;"><b>경영진 우선 확인사항</b><ol style="margin-top:5px;">{top_lines}</ol></div>
      <div style="margin-top:12px;padding:11px;background:#FFF2CC;border-left:5px solid #C65911;">
        <b style="color:#C65911;">C. Strategic Control Urgent Verification ({len(strategic_pending)}건)</b>
        <ul style="margin:6px 0 0 18px;">{strategic_lines}</ul>
        <div style="margin-top:5px;color:#666;">Direct/Top3 판정과 별개인 업무 긴급도 경보입니다. 원문 검증 후 영향도를 재판정합니다.</div>
      </div>
    </div>
    """
...
def top3_html(top3: pd.DataFrame) -> str:
    if top3.empty:
        return "<div style='padding:12px;background:#FFF7F7;border-left:5px solid #C00000;'>금일은 Direct 근거가 충분한 Top3가 없습니다. Watch 항목을 임의로 승격하지 않았습니다.</div>"
    blocks = []
    for idx, row in top3.iterrows():
        evidence = " | ".join(x for x in [
            clean(row.get("Official Evidence")), clean(row.get("Direct Evidence")), clean(row.get("Evidence")),
        ] if x) or "원문 근거 확인 필요"
        missing = clean(row.get("Missing Facts")) or "추가 확인사항 없음"
        owner = clean(row.get("Owner")) or "HQ Customs/FTA + 해당 사업부·법인"
        blocks.append(f"""
        <div style="margin:14px 0 20px;padding:16px;border-left:5px solid #C00000;background:#FFF7F7;line-height:1.6;">
          <div style="font-size:16px;font-weight:bold;">Top {idx+1}. {html_link(row.get('Headline'), row.get('URL'))}</div>
          <div style="color:#555;margin:6px 0;">{html.escape(clean(row.get('Content Type')))} | {html.escape(clean(row.get('Country')))} | {html.escape(clean(row.get('Issue')))} | Risk {html.escape(clean(row.get('Risk')))}</div>
          <div><b>1) 정책 변경 및 일정</b><br>{html.escape(short_text(row.get('Major Changes'), '변경내역 확인 필요', 1200)).replace(chr(10), '<br>')}</div>
          <div style="margin-top:8px;"><b>2) 삼성전자 관세업무 영향</b><br>{html.escape(short_text(row.get('AI Analysis'), '영향 검토 필요', 1200)).replace(chr(10), '<br>')}</div>
          <div style="margin-top:8px;"><b>3) 원문/직접 근거</b><br>{html.escape(short_text(evidence, '원문 근거 확인 필요', 900)).replace(chr(10), '<br>')}</div>
          <div style="margin-top:8px;"><b>4) 미확인 Mapping</b><br>{html.escape(short_text(missing, '추가 확인사항 없음', 700)).replace(chr(10), '<br>')}</div>
          <div style="margin-top:8px;"><b>5) 대응방안</b><br>{html.escape(short_text(row.get('Action Plan'), '담당 부서 확인 필요', 1200)).replace(chr(10), '<br>')}</div>
          <div style="margin-top:6px;"><b>Owner:</b> {html.escape(owner)}</div>
        </div>""")
    return "".join(blocks)
...
def build_html(rows: pd.DataFrame, top3: pd.DataFrame) -> str:
    subject = f"[GTI Radar] Global Trade Intelligence | {RUN_DATE}"
    regulation = rows[rows["Content Type"].eq("Regulation")]
    verification_group = regulation.get(
        "Verification Group", pd.Series("", index=regulation.index, dtype="object")
    ).astype(str)
    verified = regulation[verification_group.eq("Verified Regulation")]
    strategic_pending = regulation[verification_group.eq("Strategic Control Urgent Verification")]
    pending = regulation[~regulation.index.isin(verified.index) & ~regulation.index.isin(strategic_pending.index)]
    news = rows[rows["Content Type"].eq("News")]
    return f"""<!DOCTYPE html><html lang="ko"><head><meta charset="utf-8"><title>{html.escape(subject)}</title></head>
<body style="font-family:Arial,'Malgun Gothic',sans-serif;font-size:13px;color:#222;line-height:1.55;">
<div style="max-width:1600px;margin:0 auto;">
<h2 style="margin-bottom:3px;color:#1F4E78;">[GTI Radar] Global Trade Intelligence</h2>
...
_v334_save_excel_base = save_excel


def save_excel(rows: pd.DataFrame, top3: pd.DataFrame, paths: dict[str, Path]) -> None:
    _v334_save_excel_base(rows, top3, paths)
    watch_top3 = choose_policy_watch_top3(rows)
    book = load_workbook(paths["mail_xlsx"])
    if "Policy Watch Top3" in book.sheetnames:
        del book["Policy Watch Top3"]
    ws = book.create_sheet("Policy Watch Top3")
    export = watch_top3.copy()
    for col in OUTPUT_COLUMNS:
        if col not in export.columns:
            export[col] = ""
    ws.append(list(OUTPUT_COLUMNS))
    for values in export[OUTPUT_COLUMNS].itertuples(index=False, name=None):
...
# ======================================================================
# End of GTI STEP5 v319
# ======================================================================

def main() -> None:
    print(f"[INFO] GTI STEP5 {GTI_STEP5_VERSION} START")
    paths = output_paths()
    ...
    print(
        f"[STEP5 HTML CONSISTENCY] visible={len(rows)} / regulation={reg_n} / "
        f"news={news_n} / top3={len(top3)}"
    )
