...
GTI Radar STEP5 Mail Engine - FINAL
...
권장:
  setx GTI_SMTP_USER "kch8872@naver.com"
  setx GTI_SMTP_PASS "네이버_SMTP_앱비밀번호"
  setx GTI_SEND_EMAIL "Y"

기존 호환:
  GTI_MAIL_ID / GTI_MAIL_PW 도 자동 인식

주의:
- setx 후에는 PowerShell을 새로 열어야 반영됩니다.
- 현재 PowerShell에서만 즉시 반영하려면:
  $env:GTI_SEND_EMAIL="Y"
  $env:GTI_SMTP_USER=$env:GTI_MAIL_ID
  $env:GTI_SMTP_PASS=$env:GTI_MAIL_PW
"""
...
# 00.xlsx / mail.xlsx가 없을 때 대체 수신자. 예: lifepal.kwak@samsung.com,kch8872@naver.com
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in df.columns:
        ...
        if lc in ["date", "publish date", "publish_date", "published", "news date", "뉴스 원본 게시일시", "원문등록일", "collected_at"]:
            colmap[c] = "date"
...
def save_excel(top25: pd.DataFrame, path: str) -> None:
    ...
    widths = {"A": 6, "B": 16, "C": 18, "D": 42, "E": 48, "F": 52, "G": 46, "H": 15, "I": 28, "J": 10}
...
def top3_short(summary: str) -> str:
    return compact_text(summary)[:50]


def build_total_review(top3: pd.DataFrame) -> str:
    return "미국·EU·아시아 주요 통상 리스크가 삼성전자 생산거점과 제품별 관세·원산지 대응 필요성을 높이고 있습니다."
...
<body style="font-family:Arial,'Malgun Gothic',sans-serif;font-size:13px;color:#222;line-height:1.5;">
  <div style="max-width:1200px;margin:0 auto;">
    ...
    <ul style="margin-top:6px;margin-bottom:18px;">{bullet_lines}</ul>
...
def load_recipients() -> list[str]:
    recipients = []
    for fp in [RECIPIENT_FILE_1, RECIPIENT_FILE_2]:
        if os.path.exists(fp):
            try:
                ...
                print(f"[WARN] recipient file read failed: {fp} / {e}")
